...
def run_for(close, country):
    ...
    values, dates = align_data(df, n = 1, d = '3/20/20')

    # calculate the rolling average of the confirmed deaths
    _values, _ = rolling_average(values, dates, n = 6)

    # calculate the rolling average of the change rate
    rows, dates = rolling_average(*change_rate(values, dates), n = 6)
    ...
    plt.savefig(data.img_path + 'MotherJones5-Figure-%s-latest.png' % (country.replace(' ', '-')))
    if close:
        plt.close(fig)
